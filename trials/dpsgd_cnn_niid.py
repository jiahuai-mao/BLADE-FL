...
for _ in range(iter_num):
    ...
    subset_indices.extend(cur_samples)
...
def customize_topology():
    topology = list()
    for i in range(num_clients):
        # topology.append([j for j in range(num_clients) if i != j])
...
class Client(threading.Thread):
    ...
    def run(self):
        ...
        for client_id in self.joint_clients:
            ...
            target_client.aggregator_queue.put(gradients)
    ...
    def aggregate_and_update(self):
        ...
        while True:
            # time.sleep(0.1)
            if self.aggregator_queue.qsize() == len(self.joint_clients):
                break
        ...
        for _ in range(len(self.joint_clients)):
            tmp_gradients = self.aggregator_queue.get()
            collected_gradients.append(tmp_gradients)
...
if __name__ == "__main__":
    f = open(
        "./results/res_dpsgd_niid_{}_{}.txt".format(num_clients, num_rounds), "a+")
