...
def create_iid_splits(_, indices, num_nodes_list):
    node_indices = list()
    # flag1 = False
...
class SimpleCNN(nn.Module):
    def __init__(self, in_channels=3, hidden_size=200, num_classes=10):
        super(SimpleCNN, self).__init__()
        # Change in_channels to 3 to accommodate CIFAR-10 RGB images
        self.conv1 = nn.Conv2d(in_channels, 32, kernel_size=5, stride=1, padding=2)
        self.conv2 = nn.Conv2d(32, 64, kernel_size=5, stride=1, padding=2)
        # Update the input size of fc1 based on the CIFAR-10 dimensions
    ...
    def forward(self, x):
        ...
        # Adjust the view size to match the output shape after pooling
        x = x.view(-1, 64 * 8 * 8)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x
...
if __name__ == "__main__":
    f = open("./results/test_iid_{}_{}_{}.txt".format(num_clients, num_rounds,
