...
class model_training():
    def __init__(self, trainloader, testloader, model, device, use_DP, noise, norm, delta):
        ...
        self.optimizer = optim.SGD(
            self.net.parameters(), lr=1e-2, momentum=0.9, weight_decay=5e-4)
        ...
        if self.use_DP:
            ...
            #     secure_rng=False,
            # )
            print('noise_multiplier: %.3f | max_grad_norm: %.3f' %
                  (self.noise_multiplier, self.max_grad_norm))
            # privacy_engine.attach(self.optimizer)

        self.scheduler = lr_scheduler.MultiStepLR(
            self.optimizer, [50, 75], 0.1)
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            if str(self.criterion) != "CrossEntropyLoss()":
                targets = torch.from_numpy(
                    np.eye(self.num_classes)[targets]).float()
...
class distillation_training():
    def __init__(self, PATH, trainloader, testloader, model, teacher, device):
        ...
        self.scheduler = lr_scheduler.MultiStepLR(
            self.optimizer, [50, 100], 0.1)

    def distillation_loss(self, y, labels, teacher_scores, T, alpha):
        loss = self.criterion(F.log_softmax(y/T, dim=1),
                              F.softmax(teacher_scores/T, dim=1))
        loss = loss * (T*T * 2.0 * alpha) + \
    ...
    def train(self):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            loss = self.distillation_loss(
                outputs, targets, teacher_output, T=20.0, alpha=0.7)
            loss.backward()
            self.optimizer.step()
...
class GAN_training():
    def __init__(self, trainloader, model_discriminator, model_generator, device):
        ...
        self.optimizer_discriminator = optim.Adam(
            model_discriminator.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.optimizer_generator = optim.Adam(
            model_generator.parameters(), lr=0.0002, betas=(0.5, 0.999))
