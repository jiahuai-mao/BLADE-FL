...
def create_niid_splits_(dataset, _, num_nodes):
    """
    ...
    class_splits = np.array_split(classes, len(num_nodes))
    node_indices = []
    for num_node, class_split in zip(num_nodes, class_splits):
        node_indice = [list() for _ in range(num_node)]
        for indicator, label in zip(indices, labels):
            if label in class_split:
                x = np.random.randint(0, num_node)
                node_indice[x].append(indicator)
...
def create_niid_splits(dataset, _, num_nodes_list, alpha_list):
    """
    Splits the data among nodes in a non-IID manner.
    Each node gets data from certain classes only.
    """
    ...
    node_indices = []
    for i, (num_node, node_split) in enumerate(zip(num_nodes_list, node_splits)):
        N = num_node
        node_indice = [list() for _ in range(num_node)]
        np.random.shuffle(classes)
...
if __name__ == "__main__":
    ...
    node_indices_list = create_niid_splits(
        train_dataset, None, num_nodes_list, alpha_list
    )
