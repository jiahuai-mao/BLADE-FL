...
topology = {i: [(i - 1) % num_nodes, (i + 1) % num_nodes]
...
class AsyncNode(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_signal:
            ...
            # Communication Step (Asynchronous)
            self.communicate()
    ...
    def communicate(self):
        # Send model to neighbors without waiting
    ...
    def average_models(self):
        with self.lock:
            if self.received_models:
                ...
                for received_state in self.received_models:
                    for key in state_dict.keys():
                        state_dict[key] = alpha * received_state[key] + \
                            (1 - alpha) * state_dict[key]
...
print(f"Running AD-PSGD for {run_duration} seconds...")
